...
case object sequences {
  ...
  case object fasta {
    ...
    val rnaID: FASTA => RNAID =
    { (_:FASTA).header } andThen header.rnaID
  
    val taxonomyID: FASTA => TaxonID =
      { (_:FASTA).header } andThen header.taxonomyID
      
    val sequenceAnnotation: FASTA => SequenceAnnotation =
      { (_:FASTA).header } andThen header.sequenceAnnotation
  ...
  lazy val parseFrom: RNACentralData => Iterator[SequenceAnnotation GroupedBy RNASequence] =
